...
def main(param_file):
    ...
    gcnc_model_params = args['GCN']['gcnc_model_parameter']
    accelerator = args['GCN']['accelerator']
    max_epochs = args['GCN']['max_epochs']
    batch_size = args['GCN']['batch_size']
    ## for explainability
    ...
    for classification_dataset in classification_datasets:
        ...
        for split, (train_index, test_index) in enumerate(cv.split(df)):
            ...
            model = FragShapley.GCNClassifier(**gcnc_model_params)
            trainer.fit(model=model,
                        train_dataloaders=dl_train,)
            # save the model
            torch.save(model,os.path.join(model_results_folder, f'model_gcnc_{classification_dataset}_split_{split}.pkl'))
            
            y_pred = torch.vstack(trainer.predict(model, dl_test)).detach().cpu().numpy().squeeze()
            # performance metrics
            rows_performance.append({'model': 'GCN',
                                     'dataset': classification_dataset,
                                     'split': split,
                                     'model_params': gcnc_model_params | {'max_epochs': max_epochs, 'batch_size': batch_size},
