    ...
    for classification_dataset in classification_datasets:
        ...
        for split, (train_index, test_index) in enumerate(cv.split(df)):
            ...
            gridCV.fit(X=fps_train,
                    y=y_train)
            # save cv results
            pd.DataFrame(gridCV.cv_results_).to_pickle(os.path.join(cv_results_folder, f'cv_results_rfc_{classification_dataset}_split_{split}.pkl'))
            ...
            with open(os.path.join(model_results_folder, f'model_rfc_{classification_dataset}_split_{split}.pkl'), 'wb') as f:
