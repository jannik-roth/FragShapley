...
def main(param_file):
    ...
    gcnr_model_params = args['GCN']['gcnr_model_parameter']
    ...
    for regression_dataset in regression_datasets:
        ...
        for split, (train_index, test_index) in enumerate(cv.split(df)):
            ...
            model = FragShapley.GCNRegressor(**gcnr_model_params)
            trainer.fit(model=model,
                        train_dataloaders=dl_train,)
            # save the model
            torch.save(model,os.path.join(model_results_folder, f'model_gcnr_{regression_dataset}_split_{split}.pkl'))
            ...
            rows_performance.append({'model': 'GCN',
                                     ...
                                     'split': split,
                                     'model_params': gcnr_model_params | {'max_epochs': max_epochs, 'batch_size': batch_size},
