...
def main(param_file):
    ...
    for regression_dataset in regression_datasets:
        ...
        gridCV.fit(X=fps_train, y=y_train)
        # save cv results
        pd.DataFrame(gridCV.cv_results_).to_pickle(os.path.join(cv_results_folder, f'cv_results_rfr_{regression_dataset}.pkl'))
        ...
        with open(os.path.join(model_results_folder, f'model_rfr_{regression_dataset}.pkl'), 'wb') as f:
