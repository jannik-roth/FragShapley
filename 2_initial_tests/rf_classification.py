...
def main(param_file):
    ...
    rfc_parameter_grid = args['random_forest']['rfc_parameter_grid']
    ## for explainability
    ...
    for classification_dataset in classification_datasets:
        ...
        for split, (train_index, test_index) in enumerate(cv.split(df)):
            ...
            gridCV.fit(X=fps_train,
                    y=y_train)
            # save cv results
            pd.DataFrame(gridCV.cv_results_).to_pickle(os.path.join(cv_results_folder, f'cv_results_rfr_{classification_dataset}_split_{split}.pkl'))
            ...
            with open(os.path.join(model_results_folder, f'model_rfr_{classification_dataset}_split_{split}.pkl'), 'wb') as f:
                dump(best_clf, f, protocol=5)
            ...
            # run FragShapley
            frag_explainer = FragShapley.FragmentExplainer(model=best_clf,
            ...
            # now shap explanations
            # we explain everything in probability space here
            # Unfortunately, shap does not support the calculation in logit space
            shap_explainer = shap.TreeExplainer(model=best_clf,
                                                data=fps_train,
                                                model_output='probability', # also use log proba space here
                                                feature_perturbation='interventional',
                                                )
            shap_expl = shap_explainer.shap_values(fps_test)
            shap_expl = [i[:,1] for i in shap_expl] # only for "active" class
            ...
            results = {'dataset': classification_dataset,
                       ...
                       'fragExplainer_expected_value_logits': [ev_frag_logit for _ in smiles_test],
                       'shap_result': shap_expl,
                       'shape_expected_value': [shap_ev for _ in smiles_test],
